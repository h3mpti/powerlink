...
// find the best station for each device and output the power or no link within reach when no station is nearby
devices.map(device => {
  let bestStation = device.findBestStation(stations);
  if (bestStation && bestStation.power > 0) {
    const power = bestStation.power.toFixed(2);
    console.log(
      `Best link station for point ${device.location.x},${
        device.location.y
      } is ${bestStation.location.x},${
        bestStation.location.y
      } with power ${power}`
    );
  } else {
    console.log(
      `No link station within reach for point ${device.location.x},${
        device.location.y
      }`
    );
  }
